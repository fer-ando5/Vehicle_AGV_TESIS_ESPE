...
constexpr char TOKEN[] = "NNz7OFhy2WnVm6eDB56x";
...
void InitWiFi() {
  ...
  IPAddress staticIP(192, 168, 0, 83); // Dirección IP deseada
...
RPC_Response processSetLedMode(const RPC_Data &data) {
  ...
  //String responseJson = "{ \"newMode\": " + String(ledMode) + ", \"newoperale\": " + String(operale) + " }";
...
void processClientAttributes(const Shared_Attribute_Data &data) {
  for (auto it = data.begin(); it != data.end(); ++it) {
    ...
    if (strcmp(it->key().c_str(), encenderunled) == 0) {
      const uint16_t newoperale = it->value().as<uint16_t>();
      operale = newoperale;
      Serial.println("Operale equivale a:");
      Serial.println(operale);
    }
  }
}
...
void loop() {
  ...
  if (attributesChanged) {
      ...
      if (operale == 0){
        tb.sendTelemetryData(Haciendo, "Led apagado en teoria");
  ...
  if(operale == 1){
    tb.sendTelemetryData(Haciendo, "Led encendido en teoria");
